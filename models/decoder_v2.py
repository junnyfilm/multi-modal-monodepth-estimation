...
class Decoder_Depth(nn.Module):
    ...
    def forward(self, feats):
        ...
        out = self.conv_layers(out)                 # [bs, num_filters[-1], (H/model_scale) * (2^num_deconv), (W/model_scale) * (2^num_deconv)] -> [bs, embed_dim, (H/model_scale) * (2^num_deconv), (W/model_scale) * (2^num_deconv)]

        for _ in range(self.num_upscale_layer):
            out = self.up(out)                      # [bs, embed_dim, (H/model_scale) * (2^num_deconv), (W/model_scale) * (2^num_deconv)]   -> [bs, embed_dim, ((H/model_scale)*(2^num_deconv)) * 2, ((W/model_scale)*(2^num_deconv)) * 2]
        out = self.last_layer(out)                  # [bs, 1, H, W]
...
class Decoder_v2(nn.Module):
    def __init__(self, in_channels, out_channels, args):
        ...
        self.decoder_depth = Decoder_Depth(in_channels*2, out_channels*2, args)
    ...
    def forward(self, feat1, feat2):
        ...
        out_d1 = self.decoder_depth(feats)

        feats = torch.cat([feat2, feat1], dim=1)        # [bs, (model_scale * 32) * 2, H/model_scale, W/model_scale]
        out_r2, out_t2 = self.decoder_pose(feats)
        out_d2 = self.decoder_depth(feats)

        return out_d1, out_r1, out_t1, out_d2, out_r2, out_t2
